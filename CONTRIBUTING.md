...
### Using `llama stack show`

When installing dependencies for a distribution, you can use `llama stack show` to view and install the required packages. If you are developing with a llama-stack repository checked out and need your code to be reflected, set `LLAMA_STACK_DIR` and `LLAMA_STACK_CLIENT_DIR` to the appropriate checked out directories when running any of the `llama` CLI commands.
...
LLAMA_STACK_DIR=$(pwd) LLAMA_STACK_CLIENT_DIR=../llama-stack-client-python llama stack show --distro <distro-name>

# Install dependencies (simple approach - pipes shell commands to sh)
LLAMA_STACK_DIR=$(pwd) llama stack show --distro <distro-name> | sh

# Alternative: install dependencies separately using JSON format
LLAMA_STACK_DIR=$(pwd) llama stack show --distro <distro-name> --format json | jq -r '.pip_dependencies[]' | uv pip install -r -
LLAMA_STACK_DIR=$(pwd) llama stack show --distro <distro-name> --format json | jq -r '.special_pip_dependencies[]' | xargs -L1 uv pip install
```
