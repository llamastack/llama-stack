...
`llama-stack-spec` is a minimal dependency package that contains:
...
```bash
pip install llama-stack-spec
```

Or with uv:

```bash
uv pip install llama-stack-spec
```
...
```python
from llama_stack_spec.apis.inference import Inference, ChatCompletionRequest
from llama_stack_spec.providers.datatypes import ProviderSpec, InlineProviderSpec
from llama_stack_spec.apis.datatypes import Api
...
The main `llama-stack` package depends on `llama-stack-spec` and provides:
