...
class TestConnectorIdResolution:
    """Tests for resolving connector_id to server_url."""
    ...
    async def test_connector_id_resolved_to_server_url(self, mock_connectors_api, sample_connector):
        ...
        # Simulate the resolution logic from streaming.py
        ...
        # This is the resolution logic from _process_mcp_tool
        if mcp_tool.connector_id and not mcp_tool.server_url:
            connector = await mock_connectors_api.get_connector(mcp_tool.connector_id)
    ...
    async def test_server_url_not_overwritten_when_provided(self, mock_connectors_api, sample_connector):
        ...
        # Resolution logic should skip when server_url already exists
        if mcp_tool.connector_id and not mcp_tool.server_url:
            connector = await mock_connectors_api.get_connector(mcp_tool.connector_id)
            mcp_tool = mcp_tool.model_copy(update={"server_url": connector.url})

        # Should keep original URL
        assert mcp_tool.server_url == "http://original-server:8080/mcp"
        # Should not call connectors API
    ...
    async def test_connector_id_resolution_propagates_not_found_error(self, mock_connectors_api):
        ...
        from llama_stack_api import ConnectorNotFoundError
