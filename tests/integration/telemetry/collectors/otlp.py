...
class OtlpHttpTestCollector(BaseTelemetryCollector):
    def __init__(self) -> None:
        ...
        class _CollectorHandler(BaseHTTPRequestHandler):
            def do_POST(self_inner):  # noqa: N802 (handler signature)
                collector: OtlpHttpTestCollector = self_inner.server.collector  # type: ignore[attr-defined]
                length = int(self_inner.headers.get("content-length", "0"))
                body = self_inner.rfile.read(length)
                if self_inner.headers.get("content-encoding") == "gzip":
                    body = gzip.decompress(body)

                if self_inner.path == "/v1/traces":
                    request = ExportTraceServiceRequest()
                    request.ParseFromString(body)
                    collector._handle_traces(request)
                    self_inner._respond_ok()
                elif self_inner.path == "/v1/metrics":
            ...
            def log_message(self_inner, format, *args):  # noqa: A003 - suppress default logging
                return

            def _respond_ok(self_inner) -> None:
        ...
        self._server = _ThreadingHTTPServer(("127.0.0.1", 0), _CollectorHandler)
        ...
        for resource_spans in request.resource_spans:
            ...
            for scope_spans in resource_spans.scope_spans:
                for span in scope_spans.spans:
                    ...
                    new_spans.append(SpanStub(span.name, attributes, resource_attrs or None, events))
