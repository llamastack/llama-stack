...
def get_api_docstring(api_name: str) -> str | None:
    ...
    try:
        ...
        api_module = __import__(f"llama_stack_spec.apis.{api_name}", fromlist=[api_name.title()])
    ...
    try:
        ...
        if hasattr(config_class, "model_fields"):
            for field_name, field in config_class.model_fields.items():
                ...
                field_type = field_type.replace("llama_stack_spec.apis.inference.inference.", "")
                field_type = field_type.replace("llama_stack.providers.", "")
                field_type = field_type.replace("llama_stack_spec.providers.", "")
