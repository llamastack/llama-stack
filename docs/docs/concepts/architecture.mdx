...
- **Anthropic API** — Messages endpoint (`/v1/messages`). An adapter that translates Anthropic-format requests to the inference API. Teams using the Anthropic SDK can point it at Llama Stack without code changes. See the [Messages provider docs](/docs/providers/messages/) for current limitations.
