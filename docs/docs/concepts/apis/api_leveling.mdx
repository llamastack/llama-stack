...
| Surface | Lives under | Consumers | Levelled by | Current level |
|---|---|---|---|---|
...
> **Status:** the package today exports both surfaces flat from the top level (`from ogx_api import ...`). Splitting into `ogx_api.types` and `ogx_api.provider` sub-namespaces is in progress. Until that lands, treat any symbol re-exported from `ogx_api.__init__` as one of the two surfaces according to the rules below; sub-module imports (`from ogx_api.responses import ...`) remain unsupported and are not covered by this policy.

In both surfaces, "public" means **only** what is re-exported from `ogx_api.__init__`'s `__all__`. If a symbol you need isn't there, file a bug — do not reach into a submodule.
...
Internal helpers and base classes that are not part of either surface should not be re-exported from `ogx_api.__init__`. If a symbol must be public for an external provider but doesn't fit cleanly into the API datatype track, it belongs in the Provider SDK surface — extend `ogx_api.provider.*` rather than leaving it floating at the top level.
