...
| `max_concurrency` | `int` | No | 2 | Maximum number of concurrent requests to Docling Serve. Excess requests queue in FIFO order. Lower values protect the server from resource exhaustion on large or image-heavy files. |
| `max_queue_size` | `int` | No | 0 | Maximum number of requests waiting in the queue. 0 means unlimited. When the queue is full, new requests are rejected immediately with an error. |
