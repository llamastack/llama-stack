...
On Unix-based systems (Linux, macOS), Llama Stack can optionally use **Gunicorn with Uvicorn workers** for production-grade multi-process performance. Enable it by setting `LLAMA_STACK_ENABLE_GUNICORN=true`. This provides:

- **Multi-process concurrency**: Scales to `(2 × CPU cores) + 1` workers by default, or uses `server.workers` from config
- **Worker recycling**: Prevents memory leaks by restarting workers periodically
- **Graceful fallback**: Falls back to single-process Uvicorn if Gunicorn is unavailable

### Configuration

Enable Gunicorn and configure its behavior using environment variables:
...
- **SQLite with Multiple Workers**: SQLite works with Gunicorn's multi-process mode for development and low-to-moderate traffic scenarios. However, **SQLite only allows one writer at a time** - write operations from multiple workers are serialized. **For production deployments with high traffic, we recommend using PostgreSQL**.
