...
Llama Stack also provides an Anthropic Messages API adapter at `/v1/messages` for teams that use the Anthropic SDK. See the [Anthropic Messages API](/docs/api-anthropic-messages) docs, including a [conformance report](/docs/api-anthropic-messages/conformance).
