...
llama stack show --distro starter | sh

# Alternative: install dependencies separately using JSON format
llama stack show --distro starter --format json | jq -r '.pip_dependencies[]' | uv pip install -r -
llama stack show --distro starter --format json | jq -r '.special_pip_dependencies[]' | xargs -L1 uv pip install
