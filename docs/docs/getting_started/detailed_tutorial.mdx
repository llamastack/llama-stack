...
llm = next(m for m in models if m.id and "ollama" in m.id)
model_id = llm.id
...
# Get model to generate embeddings from text
embed_lm = next(
    m for m in client.models.list()
    if m.custom_metadata and m.custom_metadata.get("model_type") == "embedding"
)

# Create vector store using documentations below
urls = [
...
docs = []
for url in urls:
    source = f"https://raw.githubusercontent.com/pytorch/torchtune/main/docs/source/tutorials/{url}"
...
for content, metadata in docs:
  with BytesIO(content.encode()) as file_buffer:
      file_buffer.name = metadata['title']
...
# Create vector store with those files
...
# Let's test a semantic search to find some relavant docs to some query
search_response = client.vector_stores.search(
  vector_store_id=vector_store.id,
  query="What does checkpointing activation mean ?",
  max_num_results=2
)
...
llm = next(
    ...
    and m.custom_metadata.get("model_type") == "llm"
    and m.custom_metadata.get("provider_id") == "ollama"
    and "llama3.1" in m.id
)
model = llm.id # used llama3.1:8b
