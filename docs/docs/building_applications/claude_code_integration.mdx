...
Claude Code sends requests using Anthropic model names (e.g., `claude-haiku-4-5-20251001`). By default, these models don't exist in llama-stack's registry, so you need to specify which model to use:

```bash
# Option 1: Use environment variable (recommended for quick testing)
...
```bash
# Option 2: Specify model directly in Claude Code
claude --model "vllm/Qwen/Qwen3-8B" "Hello world"
```

### Advanced: Model Aliasing (Future)

:::info Coming Soon
Automatic model aliasing is under review in [PR #5471](https://github.com/llamastack/llama-stack/pull/5471). This will enable zero-config Claude Code compatibility by automatically registering Claude model names to your available providers.
:::

Once model aliasing lands, you'll be able to use Claude Code without any environment variables or model overrides:

```yaml
# Automatically in config.yaml
registered_resources:
  models:
  - model_id: claude-haiku-4-5-20251001
    provider_id: "all"  # Uses first active provider
    provider_model_id: "auto"  # Auto-selects best model
    model_type: llm
```

Then Claude Code works immediately:

```bash
export ANTHROPIC_BASE_URL="http://localhost:8321"
claude "Hello world"
# Automatically routes to your first active provider
```
...
|----------|-------------------|------------------|----------------|-------|
| **Anthropic (Bedrock)** | ✅ | ✅ | ✅ | Native Anthropic models, full feature parity |
...
| **Fireworks, Groq, Together** | ❌ (translated) | ❌ | ❌ | Works via OpenAI translation |
