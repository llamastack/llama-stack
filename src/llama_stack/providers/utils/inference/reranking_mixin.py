...
class TransformerRerankerMixin:
    async def rerank(
        self,
        model: str,
        ...
        if max_num_results is not None and max_num_results < 1:
            raise ValueError(f"max_num_results must be >= 1, got {max_num_results}")

        # Get the tokenizer and reranker model
        reranker_tokenizer, reranker_model = await self._load_reranker_model(model)

        query_text = self._extract_text(query)
        item_texts = [self._extract_text(item) for item in items]

        # Build formatted instruction pairs for each query-document combination
        pairs = [self._format_instruction(DEFAULT_RERANKER_INSTRUCTION, query_text, doc) for doc in item_texts]

        # Compute relevance scores
        relevance_scores = await asyncio.to_thread(
            self._compute_reranked_scores, reranker_tokenizer, reranker_model, pairs
        )
        ...
        if max_num_results is not None:
            indexed_scores = indexed_scores[:max_num_results]
    ...
    async def _load_reranker_model(self, model: str) -> tuple[Any, Any]:
        ...
        async with RERANKER_MODELS_LOCK:
            ...
            loaded_tokenizer, loaded_model = await asyncio.to_thread(_load_model)
            RERANKER_MODELS[model] = (loaded_tokenizer, loaded_model)
            return loaded_tokenizer, loaded_model

    @torch.no_grad()
    def _compute_reranked_scores(
        self,
