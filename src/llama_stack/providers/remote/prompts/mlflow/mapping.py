...
class PromptIDMapper:
    ...
    def extract_variables_from_tags(self, tags: dict[str, str]) -> list[str]:
        """Extract variables list from MLflow tags.

        Args:
            tags: MLflow tags dictionary
        ...
        Example:
            >>> mapper = PromptIDMapper()
            >>> mapper.extract_variables_from_tags({"variables": "var1,var2"})
            ["var1", "var2"]
        """
        variables_str = tags.get("variables", "")
        if not variables_str:
            return []

        # Split comma-separated string and filter empty strings
        return [v.strip() for v in variables_str.split(",") if v.strip()]
