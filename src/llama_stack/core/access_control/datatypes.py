...
class EndpointScope(BaseModel):
    """Scope for endpoint-level access control.

    Defines which API endpoints can be accessed. The paths field
...
class EndpointAccessRule(BaseModel):
    """Endpoint-level access rule for controlling API endpoint access.

    This rule defines which API endpoints users can access based on their
    attributes (roles, teams, etc). Rules are evaluated before resource-level
    access control.

    A rule defines either permit or forbid access to specific endpoints. The endpoints
    ...
    - Full wildcard: "*" matches all endpoints
    ...
    If no endpoint_policy is configured, all endpoints are allowed.
    If endpoint_policy is configured, rules are tested in order to find a match.
    ...
    - permit:
      ...
      description: users can access files and models endpoints

    - permit:
        paths: "*"
      when: user with admin in roles
      description: admins have access to all endpoints
    """

    permit: EndpointScope | None = None
    forbid: EndpointScope | None = None
    when: str | list[str] | None = None
    unless: str | list[str] | None = None
    description: str | None = None
